...
void sort(matrixElement *matrix, int lineOrColumn){ /*0 - line, 1 - column*/
  switch(lineOrColumn){
    case 0:{ /*Sort by line first*/
      ...
      for (i=0;i<lastElement;i++){
        for (f = i; matrix[f].line == matrix[i].line; f++)
        l_sup = f; /*Finds last position of same line*/
    ...
    case 1:{ /*Sort by column first*/
      ...
      for (i=0;i<lastElement;i++){
        for (f = i; matrix[f].column == matrix[i].column; f++)
        l_sup = f; /*Finds last position of same column*/
